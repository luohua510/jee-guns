...
import hh.jee.guns.lang.convert.NumberChineseFormatter;
...
public class ChineseDate {
	...
	public String getFestivals() {
		...
		for (String fv : lFtv) {
			final List<String> split = StringUtil.split(fv, ' ');
			if (split.get(0).contentEquals(currentChineseDate)) {
				getFestivalsList.add(split.get(1));
			}
		}
		return StringUtil.join(",", getFestivalsList);
	}
