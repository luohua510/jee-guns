package hh.jee.guns.net.url;


import hh.jee.guns.lang.convert.Convert;
import hh.jee.guns.lang.map.TableMap;
import hh.jee.guns.util.*;
...
public class UrlQuery {
	...
	public UrlQuery parse(String queryStr, Charset charset) {
		...
		for (i = 0; i < len; i++) {
			...
			if (c == '=') { // 键值对的分界点
				...
				if (null == name && pos != i) {
					...
					addParam(queryStr.substring(pos, i), StringUtil.EMPTY, charset);
				} else if (name != null) {
		...
		if (pos != i) {
			...
			addParam(name, StringUtil.EMPTY, charset);
		}
		return this;
	}
	...
	public String build(Charset charset) {
		...
		for (Map.Entry<CharSequence, CharSequence> entry : this.query) {
			...
			if (StringUtil.isNotEmpty(key)) {
				sb.append(URLUtil.encodeAll(StringUtil.str(key), charset)).append("=");
				value = entry.getValue();
				if (StringUtil.isNotEmpty(value)) {
					sb.append(URLUtil.encodeAll(StringUtil.str(value), charset));
				}
			}
		}
		return sb.toString();
	...
	private static String toStr(Object value) {
		...
		if (value instanceof Iterable) {
			...
			result = IteratorUtil.join((Iterator<?>) value, ",");
		} else {
